...
func (s *ClassroomService) createClassroom() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		// Convert student team name to slug formatting
		...
		studentTeam, err := s.appClient.CreateTeam(c.Context(), classroomData.OrgName, studentTeamName, &description, maintainers)
		if err != nil {
			return errs.InternalServerError()
		}
...
func (s *ClassroomService) sendOrganizationInvitesToRequestedUsers() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		for _, classroomUser := range classroomUsers {
			...
			modifiedClassroomUser, err := s.inviteUserToOrganization(c.Context(), s.appClient, classroom.OrgName, classroomID, classroomRole, classroomUser.User)
...
func (s *ClassroomService) sendOrganizationInviteToUser() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		// use the current user's client to invite the user to the organization
		invitee, err = s.inviteUserToOrganization(c.Context(), s.appClient, classroom.OrgName, classroomID, classroomRole, invitee.User)
		if err != nil {
			return errs.InternalServerError()
		}
...
func (s *ClassroomService) revokeOrganizationInvite() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		err = s.appClient.CancelOrgInvitation(c.Context(), classroom.OrgName, classroomUser.GithubUsername)
		if err != nil {
			return errs.InternalServerError()
		}
