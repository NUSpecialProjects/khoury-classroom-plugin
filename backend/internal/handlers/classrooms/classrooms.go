...
func (s *ClassroomService) createClassroom() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		existingTeam, err := s.githubappclient.GetTeamByName(c.Context(), classroomData.OrgName, teamName)
		if err == nil && existingTeam != nil {
			// Team exists - delete it first
			err = s.githubappclient.DeleteTeam(c.Context(), *existingTeam.ID)
			if err != nil {
				return errs.InternalServerError()
			}
		}
		...
		studentTeam, err := s.githubappclient.CreateTeam(c.Context(), classroomData.OrgName, teamName, &description, maintainers)
		if err != nil {
			return errs.InternalServerError()
		}
